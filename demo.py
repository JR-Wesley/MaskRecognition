...
def load_tensor(path):
    """从文件读取 tensor 对象"""
    """add map_location = 'cpu' if running a cpu-only machine"""
    return torch.load(gzip.GzipFile(path, "rb"), map_location=torch.device('cpu'))
...
def eval_model():
    ...
    # model.load_state_dict(torch.load("retrain_model113.pt", map_location="cpu"))
